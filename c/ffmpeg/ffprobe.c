...
int ffprobe( const char* filename, char* info){
    const Writer *w;
    WriterContext *wctx;
    int ret;
    ...
    av_log(NULL, AV_LOG_DEBUG, "get_probe:filename is [%s]",filename); 

    writer_register_all();
    w = writer_get_by_name("json");
    av_log(NULL, AV_LOG_DEBUG, "get_probe:writer_get_by_name is [0x%x]",w); 
    

    if ((ret = writer_open(&wctx, w, NULL, NULL)) >= 0) {
        av_log(NULL, AV_LOG_DEBUG, "get_probe:writer_open is ok"); 
        ...
        if (filename) {
            ...
            av_log(NULL, AV_LOG_DEBUG, "get_probe:probe_file ret[%d]",ret); 
        }
    ...
    av_log(NULL, AV_LOG_DEBUG, "ffprobe:info[%s]",info);
