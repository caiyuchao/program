...
void av_noreturn exit_program(int ret)
{
    ...
    gexit = ret; /* not all OS-es handle main() return value */
    pthread_exit(NULL);
}
...
//                fprintf(stderr, "drift:%f len:%d opts:%"PRId64" ipts:%"PRId64" fifo:%d\n", delta, -1, ost->sync_opts, (int64_t)(get_sync_ipts(ost) * enc->sample_rate), av_fifo_size(ost->fifo)/(ost->st->codec->channels * 2));
        ...
        ost->sync_opts = llrintf(get_sync_ipts(ost) * enc->sample_rate) -
    ...
    if (format_video_sync != VSYNC_PASSTHROUGH) {
        ...
        if (vdelta < -1.1)
            ...
            if (vdelta <= -0.6) {
                ...
                ost->sync_opts = llrintf(sync_ipts);
        } else if (vdelta > 1.1)
            nb_frames = llrintf(vdelta);
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (qp_hist) {
                ...
                if (qp >= 0 && qp < FF_ARRAY_ELEMS(qp_histogram))
                    qp_histogram[qp]++;
                for (j = 0; j < 32; j++)
                    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "%X", (int)llrintf(log(qp_histogram[j] + 1) / log(2)));
            }
            if (enc->flags&CODEC_FLAG_PSNR) {
                int j;
...
static int thread_main(arg_context *arg)
{
    int argc;
    char **argv;
    OptionsContext o = { 0 };
    ...
    for(i=0; i<argc; i++){
    	av_log(NULL, AV_LOG_INFO, "argv[%d]:[%s]\n",i, argv[i]);
    }
...
int ffmpeg(char *cmd){
...
#ifdef ANDROID
	av_log_set_callback(av_log_callback_android);
#endif
    ...
    if ( pthread_create( &mythread, NULL, thread_main, &arg) ) {
        av_log(NULL, AV_LOG_FATAL, "error creating thread.");
        return 1;
    }
    if ( pthread_join ( mythread, NULL ) ) {
         av_log(NULL, AV_LOG_FATAL, "error joining thread.\n");
         return 1;
    }
    if(gexit == -1) gexit = 0;
